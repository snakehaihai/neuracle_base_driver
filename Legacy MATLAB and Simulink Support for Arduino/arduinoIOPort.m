...
%> Modified from https://github.com/iandol/opticka/tree/master/communication/arduinoIOPort
% ========================================================================
...
% Also added a timedTTL function, requiring a compatible arduino sketch: adio.ino (adio_ian.pde)
% Added a timedStrobedWord function to allow sending timed strobed word,
% requring adio_ian_hzl_uno.ino (>=20230921) %% HE Zhenliang
	...
	methods
		...
		function a=arduinoIOPort(port,endPin,startPin)
			...
			fprintf('===> It took %.3f secs to establish response\n',tout);
			% check returned value
			if a.sktc==0
				disp('===> Basic Analog and Digital I/O (adio_ian_hzl_uno.pde >=20230921) sketch detected !');
			elseif a.sktc==1
			...
			if nargin == 3
				if ischar(str)
				    if strcmp(str, 'output')
                        val = 1; 
                    elseif strcmp(str, 'input')
                        val = 0;
                    elseif strcmp(str, 'input_pullup')
                        val = 2;
                    else
                        val = 0;
                    end
		...
		% digital read
		function val=digitalRead(a,pin, input_pullup)
			...
			% modified by HE Zhenliang
            %
			% Examples:
			% val=digitalRead(a,4); % reads pin #4
            % val=digitalRead(a,4,0); % reads pin #4
            % val=digitalRead(a,4,1); % reads pin #4, which is in INPUT_PULLUP mode
            ...
            if nargin == 2 || input_pullup == 0
                val = str2double(char(val(1)));
            elseif nargin == 3 && input_pullup == 1
                if ~isempty(val)
                    val = val(end);
                end
            end
		...
		%===================================================strobeWord
        function timedStrobedWord(a,value,durStrobedWord,durSettle)
			% timedStrobedWord(a, value, durStrobedWord, dursettle) -- send a byte [0-255] value to the 8 pins, with
			% a long low-high transition time (durStrobedWord, in ms, max=4,294,967.295ms), followed by a long high-low
            % transition time (durSettle in ms, max=4,294,967.295ms), without blocking matlab
			% pins 2-7 as strobed words
            % added by HE Zhenliang
			if a.isDemo; return; end
			if value < 0; value = 0; end
			if value > 255; value = 255; end

            if durStrobedWord <= 0; return; end
			if durStrobedWord > 2^32-1; durStrobedWord = 2^32-1; end
			durStrobedWord = typecast(uint32(durStrobedWord*1000),'uint8'); %convert to 4 uint8 bytes

            if durSettle <= 0; durSettle = 0; end
			if durSettle > 2^32-1; durSettle = 2^32-1; end
			durSettle = typecast(uint32(durSettle*1000),'uint8'); %convert to 4 uint8 bytes

			% send mode and pin
			IOPort('Write',a.conn,uint8([55 value durStrobedWord(1) durStrobedWord(2) durStrobedWord(3) durStrobedWord(4) ...
                durSettle(1) durSettle(2) durSettle(3) durSettle(4)]),1);
        end
